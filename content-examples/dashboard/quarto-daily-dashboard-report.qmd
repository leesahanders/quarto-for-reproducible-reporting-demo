...
rss_r_journal <- tidyfeed(rss_url) %>% slice_head(n=5) %>% select(entry_title, entry_last_updated) %>% mutate(entry_last_updated = as.Date(entry_last_updated))

rss_r_journal_url <- tidyfeed(rss_url) %>% slice_head(n=5) %>% select(entry_link)

rss_r_journal %>%
  kbl(booktabs = T, align = "lc") %>% #, caption = "RSS Feed for R Journal") %>%
  kable_styling() %>%
  kable_paper(full_width = T) %>%
