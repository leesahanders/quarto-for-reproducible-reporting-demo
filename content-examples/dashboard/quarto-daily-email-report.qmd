---
title: "Quarto Daily Dashboard"
author: "Lisa Anders" 
format: email
editor: source
theme:
  light: [custom, custom.scss]
  dark: [vapor, custom.scss]
css: styles.css
---
...
# Captions: https://biostats-r.github.io/biostats/quarto/04-figures-tables.html
```
Report date
...
df <- tibble(state = state, uv_index = uv_index) %>%
  mutate(
...
kable(df, align = 'c', booktabs = TRUE, caption = "UV Index") 
...
#| warning: false
#| message: false
#| label: tbl-posit-product
#| tbl-cap: Current product releases 
#| cap: Current product releases 
...
rss_posit_blog <- tidyfeed(rss_url) %>% slice_head(n=5) %>% select(item_title, item_pub_date)
rss_posit_blog_url <- tidyfeed(rss_url) %>% slice_head(n=5) %>% select(item_link)

rss_posit_blog %>%
  kbl(booktabs = T, align = "c", caption = "RSS Feed for Posit Blog posts") %>%
  kable_styling() %>%
  kable_paper(full_width = T) %>%
...
rss_r_journal <- tidyfeed(rss_url) %>% slice_head(n=5) %>% select(entry_title, entry_last_updated)
rss_r_journal_url <- tidyfeed(rss_url) %>% slice_head(n=5) %>% select(entry_link)

rss_r_journal %>%
  kbl(booktabs = T, align = "c", caption = "RSS Feed for R Journal") %>%
  kable_styling() %>%
  kable_paper(full_width = T) %>%
  column_spec(1, link = rss_r_journal_url$entry_link)

# entry_title, entry_url, entry_link, entry_last_updated
```
...
rss_r_weekly %>%
  kbl(booktabs = T, align = "c", caption = "RSS Feed for r-weekly") %>%
  kable_styling() %>%
  kable_paper(full_width = T) %>%
...
car_tbl_cap <- paste(car_recalls_results$ModelYear[1], car_recalls_results$Make[1], car_recalls_results$Model[1])
...
`r paste(car_recalls_results$ModelYear[1], car_recalls_results$Make[1], car_recalls_results$Model[1])` 
...
<!-- https://github.com/quarto-dev/quarto-r/issues/137#issuecomment-1966765073 -->

<!-- https://github.com/rstudio/docs.rstudio.com/pull/1530 -->

<!-- https://github.com/rich-iannone/quarto-email/issues/9 -->

<!-- https://github.com/quarto-dev/quarto-r/issues/137 -->
