...
for(i in 1:nrow(us_cities_sample)){
  ...
  if(!inherits(possibleError, "error")){
    ...
    # uv_hourly_tmp <- fromJSON(uv_hourly_raw[[1]]) %>% as.data.frame
    # uv_hourly_tmp <- fromJSON(uv_hourly_raw) %>% as.data.frame
    # uv_hourly_tmp <- enframe(unlist(uv_hourly_raw)) %>% 
    #   as.data.frame %>% 
    #   separate(name, into = c(paste0("x", 1:5)), fill = "right")
    # uv_hourly_tmp <- as.data.frame(uv_hourly_raw, optional = TRUE) 
    # class(uv_hourly_raw) <- "json"
    # sample_data <- jsonlite::fromJSON(uv_hourly_raw)
    # uv_hourly_tmp <- as.data.frame(uv_hourly_raw, optional = TRUE) 
    uv_hourly_tmp <- as.data.frame(unlist(uv_hourly_raw, recursive = FALSE) , optional = TRUE) 
